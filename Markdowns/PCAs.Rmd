...
estrutura_loadings_filtrados_PC9 <- estrutura_loadings[which(estrutura_loadings[,9] > 0.2 | estrutura_loadings[,9] < -0.2),9]
estrutura_loadings_filtrados_PC9
...
cumulative_importance_bacia <- rep(NA, length(importance_bacia))
exp_importance_bacia <- rep(NA, length(importance_bacia))

cumulative_importance_bacia[1] <- importance_bacia[1]
#exp_importance_bacia[1] <- importance_bacia[1]
for(i in 2:length(importance_bacia)){
  cumulative_importance_bacia[i] <- cumulative_importance_bacia[i-1] + importance_bacia[i]
#  exp_importance_bacia[i] <- importance_bacia[i-1]/2
}


Eigenvalues_bacia <- data.frame(autovalores = pca_bacia$CA$eig,
                                    importance = importance_bacia,
                                cumulative_importance_bacia)

Eigenvalues_bacia

screeplot(pca_bacia, bstick = TRUE, type = "lines", 
          main = "Scree Plot vs Broken Stick Model")
...
barplot(Eigenvalues_bacia$importance)
